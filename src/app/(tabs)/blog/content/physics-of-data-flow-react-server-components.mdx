...
There are a few optimizations that React does when it comes to data fetching in Server Components.

##### Fetch De-duplication

React makes sure that, within a single render cycle, it performs a fetch only once and reuses the response for identical
fetch calls in other places.

However, for this to work, you must ensure that the request is truly identical. This includes having the same headers
and request configuration. In real-world applications, this is often not the case.
...
With this logic, if you use `getUserPreferences` or `getOrderStatus` at multiple places, React won’t be able to
de-duplicate the requests. For React, each call to `getUserPreferences` is a unique fetch because the headers are
different, and `getOrderStatus` is custom logic, so React has no way of knowing whether it should de-duplicate these
calls.
...
that won’t change during the session. It’s small, predictable, and requires extra effort.
...
you revalidate the client cache using your framework’s invalidation APIs (for example, revalidatePath or router.refresh
in Next.js). This keeps the update flow consistent.
...
export const metadata = {
  ...
  description:
    "A deep dive into how data flows in React Server Component architectures. This article explores server-owned, client-owned, and shared data patterns, explains the trade-offs behind each approach, and provides practical guidance for designing scalable RSC applications.",
  metaDescription:
    "Learn how data flows in React Server Components. Understand server vs client data ownership, shared data patterns, hydration trade-offs, and how to design scalable RSC architectures without relying on fragile APIs.",
};
